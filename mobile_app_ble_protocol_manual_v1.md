# 手机 APP BLE 接入协议与操作手册 V1

适用固件基线：当前工作区 `EEG2channel`，截至 2026-06-12 的 BLE 结构。

目标读者：手机 APP 开发、联调、测试、产品。

当前设计定位：
- 手机 APP 不接收也不展示原始 EEG 波形。
- 手机 APP 主要使用 `FFF1` 状态流、`FFF2` 控制流，必要时使用 `FFF3` IMU 流。
- 如需专注/疲劳指标，当前固件必须订阅 `FFF4`，因为指标与 EEG 同帧封装在 `FFF4` 内。
- 手机 APP 可以在 `FFF4` 完整重组后，只解析帧头与 `focus/fatigue`，忽略后续 EEG 数组。
...
1. 扫描并连接设备。
2. 订阅 `FFF1`，进入检查页。
3. 可选订阅 `FFF3`，显示头部姿态和动作事件。
4. 等待 `FFF1` 中佩戴和阻抗状态满足要求。
5. 向 `FFF2` 发送开始采集命令。
6. 采集开始后，订阅 `FFF4`。
7. 前 5 秒显示“预热中”，不展示专注/疲劳正式结果。
8. 5 秒后从 `FFF4` 读取有效的专注/疲劳指标。
9. 停止采集时，向 `FFF2` 发送停止命令，界面回到检查页。
...
说明：
- 当前手机 APP 如果只关心状态，不必订阅 `FFF3`。
- 当前手机 APP 如果需要专注/疲劳，必须订阅 `FFF4`。
- 当前固件没有单独的“指标专用特征”；`focus/fatigue` 与 EEG 同帧打包。
...
APP 侧建议：
- 点击“开始采集”后，不要立即把 UI 当成“指标已有效”。
- 等待 `FFF1.device_mode == RUNNING`，并进入 `FFF4` 预热计时。
...
如果手机 APP 不展示姿态，则可不订阅 `FFF3`，以减少流量和处理压力。
...
当前手机 APP 虽然“不接收 EEG 显示”，但如果需要专注/疲劳，仍然必须订阅 `FFF4`。
...
## 10. APP 端关于 FFF4 的实现建议

### 10.1 强烈建议

- Android 端尽量请求更大的 MTU，例如 `247`
- 不然 `FFF4` 分片数会明显增多，重组压力更大
...
重组成功后：
- 若 `metrics_update_count == 0`
  - 显示“预热中”
  - 不更新正式专注/疲劳图表
- 若 `metrics_update_count > 0`
  - 用完整帧中的 `focus/fatigue` 更新图表

### 10.4 如果 APP 完全不想处理大流量 EEG
...
- 仍要订阅 `FFF4`
- 仍要重组完整帧
- 只是解析完 `focus/fatigue` 后丢弃 EEG 数组

这在功能上可行，但在流量和功耗上不是最优。

后续若要进一步优化，建议固件增加一个专门的“指标通道”，例如未来扩展 `FFF5`：
- 单独下发 `focus/fatigue + timestamp + sequence`
- 手机 APP 就无需再接收 `FFF4`

---

## 11. 当前版本下手机 APP 最终可达到的效果

在不处理 EEG 可视化的前提下，手机 APP 现在应能实现：
...
这比“一连接就直接给指标”更符合医疗/工业设备的真实工作逻辑。
